...
import { TransactionDateTypeEnum, TransactionTypeEnum, type DepositRecord } from '@pork-buns/core/types/transactionRecord';
...
const columns: QTableColumn<DepositRecord>[] = [
  // { name: 'date', label: '日期', align: 'left', field: 'CreateDate', format: (val) => $date(val) },
  { name: 'no', label: '订单编号', align: 'left', field: 'BillNo' },
  // { name: 'createDate', label: '充值时间', field: 'CreateDate' },
  { name: 'amount', label: '充值金额', field: 'Amount' },
  { name: 'channel', label: '充值渠道', field: 'DepositTypeName', format: (val: string) => $i18n.t(`transactionRecord.DepositGroup.${val}`) },
...
const $recordLoader = useRowsLoader([] as DepositRecord[], (ctx) => {
  ctx.setSize(20);
  ctx.setIndex(1);
  ctx.setMinIndex(1);
